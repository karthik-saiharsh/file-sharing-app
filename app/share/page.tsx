...
const page = () => {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!file) toast.error("Please upload a file");
    else if (key1?.trim() != key2?.trim()) toast.error("Keys must match");
    else if (key1.trim() == "" || key2.trim() == "") toast.error("Key can't be empty");
    else if (file.name.length > 100) toast.error("Your File Name cannot be longer than 100 characters");
    else if ((file.size / 1024) / 1024 > 4.50) toast.error("Upload Files Smaller than 4.5MB");
    else {
      ...
      if (res_json.success) {
        toast.success("File Uploaded Successfully!", {
          description: `Your FileID is ${res_json.fileid}. You will need this to receive your file.`,
        });
      }
      else {
