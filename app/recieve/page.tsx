...
const page = () => {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (reqType == "download") {
      if (!res.ok) {
        if (res.status === 401) toast.error("Invalid Key - Please check your key and try again");
        else if (res.status === 404) toast.error("File not found. Please check your File ID.");
        else toast.error("Operation Failed. Could not download File");
        return;
      }
      ...
      if (res.ok) {
        ...
        toast.error(errorMsg || "Operation Failed. Could not delete File");
      }
    } else {
